...
namespace 应用程序域
{
    ...
    namespace gts
    {
        public class mc
        {
            ...
            [DllImport("gts.dll")]
            public static extern short GT_CompareStatus(out short pStatus, out ushort pCount);
            [DllImport("gts.dll")]
    ...
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            int no = 0;
            Console.WriteLine(Interlocked.Increment(ref no) % 1000);
            Console.WriteLine(Interlocked.Increment(ref no) % 1000);
            Console.WriteLine(Interlocked.Increment(ref no) % 1000);
            Console.WriteLine(Interlocked.Increment(ref no) % 1000);
            Console.WriteLine(Interlocked.Increment(ref no) % 1000);
            var log = LogManager.GetLogger("卧槽");
            new TestClass().Test();
